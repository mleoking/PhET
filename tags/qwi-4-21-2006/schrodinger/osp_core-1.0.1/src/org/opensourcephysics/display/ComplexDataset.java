...
public class ComplexDataset extends AbstractTableModel implements Drawable, Measurable {
  ...
  private String xColumnName;  // the name of the x data
  private String reColumnName; // the name of the y data
  private String imColumnName; // the name of the y data
  private int stride = 1;      // the data stride in table view
  private AffineTransform flip;
  private ComplexColorMapper colorMap = new ComplexColorMapper(1);
...
*/
  ...
  public void setXYColumnNames(String _xColumnName, String _reColumnName, String _imColumnName) {
    ...
    reColumnName = _reColumnName;
    imColumnName = _imColumnName;
  }
  ...
  protected void drawPhaseCurve(DrawingPanel drawingPanel, Graphics2D g2) {
    ...
    for(int i = 0;i<index;i++) {
      ...
      if(y>0) {
        g2.setColor(colorMap.complexToColor((oldRe+re)/2, (im+oldIm)/2));
      }
  ...
  protected void drawPhaseBars(DrawingPanel drawingPanel, Graphics2D g2) {
    ...
    for(int i = 0;i<index;i++) {
      ...
      double im = im_points[i];
      double y = Math.sqrt(re*re+im*im);
      g2.setColor(colorMap.complexToColor(re, im));
  ...
  protected void drawPhasePosts(DrawingPanel drawingPanel, Graphics2D g2) {
    ...
    for(int i = 0;i<index;i++) {
      ...
      double im = im_points[i];
      double y = Math.sqrt(re*re+im*im);
      drawPost(drawingPanel, g2, xpoints[i], y, postWidth, colorMap.complexToColor(re, im));
    }
  }
