...
public class SceneGraphPanel extends JPanel {
    ...
    public DirtyRegionSet collectDirtyRegions() {
        DirtyRegionSet dirtyRegionSet = new DirtyRegionSet();
        rootGraphic.collectDirtyRegions( dirtyRegionSet );
        return dirtyRegionSet;
    }

    public void repaintDirtyRegions( DirtyRegionSet r ) {
        ArrayList regions = r.getRegions();
        for( int i = 0; i < regions.size(); i++ ) {
            Rectangle2D x = (Rectangle2D)regions.get( i );
            Rectangle z = RectangleUtils.toRectangle( x );
...
//            System.out.println( "handler= " + graphic );
    ...
    protected void paintComponent( Graphics g ) {
        ...
        RenderEvent renderEvent = new RenderEvent( (Graphics2D)g );
        rootGraphic.render( renderEvent );
