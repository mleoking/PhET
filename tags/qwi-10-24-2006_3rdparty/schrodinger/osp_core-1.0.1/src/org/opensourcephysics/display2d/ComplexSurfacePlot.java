...
public class ComplexSurfacePlot implements Plot2D, False3D {
  ...
  private void generateVerticesFromArray(ArrayData griddata) {
    ...
    for(int ix = 0;ix<numCols;ix++) {
      ...
      for(int iy = 0;iy<numRows;iy++) {
        ...
        vertexArray[ix*numRows+iy] = new ComplexSurfaceVertex(-10+(x-xmin)*xfactor, -10+(y-ymin)*yfactor, ampdata[ixd][iyd], redata[ixd][iyd], imdata[ixd][iyd], this);
        y += dy;
      }
      x += dx;
    }
  ...
  private void generateVerticesFromPoints(GridPointData griddata) throws IllegalArgumentException {
    ...
    for(int ix = 0;ix<numCols;ix++) {
      double y = ymin; // bottom;
      for(int iy = 0;iy<numRows;iy++) {
        vertexArray[ix*numRows+iy] = new ComplexSurfaceVertex(-10+(x-xmin)*xfactor, -10+(y-ymin)*yfactor, data[ix][iy][ampIndex], data[ix][iy][reIndex], data[ix][iy][imIndex], this);
        y += dy;
      }
      x += dx;
    }
  ...
  private void generateVerticesFromPoints_old(GridPointData griddata) {
    ...
    if(griddata.getDy()>0) {
      for(int ix = 0;ix<numCols;ix++) {
        for(int iy = 0;iy<numRows;iy++) {
          vertexArray[ix*numRows+iy] = new ComplexSurfaceVertex(-10+(data[ix][iy][0]-xmin)*xfactor, -10+(data[ix][iy][1]-ymin)*yfactor, data[ix][iy][ampIndex], data[ix][iy][reIndex], data[ix][iy][imIndex], this);
        }
      }
    } else {
      for(int ix = 0;ix<numCols;ix++) {
        for(int iy = 0;iy<numRows;iy++) {
          vertexArray[ix*numRows+iy] = new ComplexSurfaceVertex(-10+(data[ix][numRows-iy-1][0]-xmin)*xfactor, -10+(data[ix][numRows-iy-1][1]-ymin)*yfactor, data[ix][numCols-iy-1][ampIndex], data[ix][numCols-iy-1][reIndex], data[ix][numCols-iy-1][imIndex], this);
        }
      }
    }
    data = null;
