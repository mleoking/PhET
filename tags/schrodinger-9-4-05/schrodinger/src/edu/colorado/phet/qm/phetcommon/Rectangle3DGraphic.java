...
public class Rectangle3DGraphic extends CompositePhetGraphic {
    ...
    public Rectangle3DGraphic( Component component, Rectangle rectangle, Paint face, Stroke stroke, Paint top, Paint right, int dx, int dy, Paint borderPaint ) {
        ...
        this.dx = dx;
        this.dy = dy;
        RoundRectangle2D roundRectangle2D = new RoundRectangle2D.Double( rectangle.x, rectangle.y, rectangle.width, rectangle.height, rectangle.width / 10, rectangle.height / 10 );
        faceGraphic = new PhetShapeGraphic( component, roundRectangle2D, face, stroke, borderPaint );
    ...
    private void update() {
        if( rectangle != null ) {
            ...
            RoundRectangle2D roundRectangle2D = new RoundRectangle2D.Double( rectangle.x, rectangle.y, rectangle.width, rectangle.height, rectangle.width / 6.0, rectangle.height / 6.0 );
            faceGraphic.setShape( roundRectangle2D );
            setBoundsDirty();
            autorepaint();
        }
    }
