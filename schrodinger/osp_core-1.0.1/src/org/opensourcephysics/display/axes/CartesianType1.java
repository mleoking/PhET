...
public class CartesianType1 implements CartesianAxes, Dimensioned {
  ...
  // W. Christian changed font name Helvetica to Dialog for better unicode support.
  ...
  /** axes will be drawn only if visible is true */
  ...
  protected void drawPlot(DrawingPanel panel, Graphics graphics) {
    ...
    int xStartPosition = panelWidth-5;
    if(xlog) {
    ...
    if(yticks==null) {
      ...
      for(double ypos = yTmpStart;ypos<=ytickMax;ypos = gridStep(ygrid, ypos, yTickSize, ylog)) {
        ...
        int yCoord1 = 0;
        if(!ylog&&(yExponent>0)||(yExponent<0)) {
    ...
    if(xticks==null) { // auto-ticks
      ...
      xTickSize = (xlog) ? roundUp(0.8*(xtickMax-xtickMin)/(double) numberXTickMarks) : roundUp((xtickMax-xtickMin)/(double) numberXTickMarks);
      numfracdigits = numFracDigits(xTickSize);
      ...
      for(double xpos = xTmpStart;xpos<=xtickMax;xpos = gridStep(xgrid, xpos, xTickSize, xlog)) {
        ...
        if(!xlog&&(xExponent>1)||(xExponent<-1)) {
          // xCoord1 = panel.xToPix(xpos * Math.pow(10, xExponent));
