...
class RobotMovingCompanyGameModel(val model: MotionSeriesModel,
  ...
  def resetAll() = {
    model.surfaceSegments(1).setAngle(0)
    model.walls = false
    model.surfaceSegments(0).startPoint = new Vector2D(-10, 0).rotate(-initAngle)
    model.surfaceSegments(0).startPoint = new Vector2D(-10, 0).rotate(-initAngle)
    _launched = false
    resultMap.clear()
    setObjectIndex(0)
  ...
  def setObjectIndex(newIndex: Int) = {
    ...
    _bead = model.createBead(-model.surfaceSegments(0).length, sel.width)
