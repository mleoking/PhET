...
class RobotMovingCompanyCanvas(model: MotionSeriesModel,
  ...
  def changeY(dy: Double) = {
    val result = model.surfaceSegments(0).startPoint + new Vector2D(0, dy)
    if (result.y < 1E-8)
  ...
  def updatePosition(dy: Double) = {
    model.surfaceSegments(0).startPoint = changeY(dy)
    model.bead.setPosition(-model.surfaceSegments(0).length)
  }
  ...
  override def createLeftSegmentNode = new RampSegmentNode(model.surfaceSegments(0), transform, model)

  override def createRightSegmentNode = new RampSegmentNode(model.surfaceSegments(1), transform, model)
  ...
  override def createEarthNode = new EarthNodeWithCliff(transform, model.surfaceSegments(1).length, gameModel.airborneFloor)
}
