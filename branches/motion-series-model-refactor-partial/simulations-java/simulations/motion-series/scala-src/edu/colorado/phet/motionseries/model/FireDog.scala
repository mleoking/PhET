...
class Raindrop(p: Vector2D, rainSpeed: Double, angle: Double, rampModel: MotionSeriesModel) {
  ...
  def stepInTime(dt: Double) = {
    ...
    val intersection = MathUtil.getLineSegmentsIntersection(origPosition, newPosition, rampModel.surfaceSegments(1).startPoint, rampModel.surfaceSegments(1).endPoint)
