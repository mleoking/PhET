...
abstract class MotionSeriesCanvas(model: MotionSeriesModel,
  ...
  def createLeftSegmentNode = new RampSegmentNode(model.surfaceSegments(0), transform, model)

  def createRightSegmentNode: HasPaint = new RotatableSegmentNode(model.surfaceSegments(1), transform, model)
  ...
  private def compositeListener(listener: () => Unit) = {
    model.surfaceSegments(0).addListener(listener)
    model.surfaceSegments(1).addListener(listener)
  }
...
abstract class MotionSeriesCanvasDecorator(model: MotionSeriesModel,
  ...
  val pusherNode = new PusherNode(transform, model.bead, model.pusher)
...
class RampCanvas(model: MotionSeriesModel, coordinateSystemModel: AdjustableCoordinateModel, freeBodyDiagramModel: FreeBodyDiagramModel,
  ...
  def addHeightAndAngleIndicators() = {
    playAreaNode.addChild(new RampHeightIndicator(model.surfaceSegments(1), transform))
    playAreaNode.addChild(new RampAngleIndicator(model.surfaceSegments(1), transform))
  }
}