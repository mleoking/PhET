...
class RampControlPanelBody(model: MotionSeriesModel,
  ...
  if (showFrictionControl) {
    ...
    def getSegmentIcon(_frictionless: Boolean) = {
      ...
      val segment = new SurfaceSegment(new Point2D.Double(0, 0), new Point2D.Double(3, 0))
  ...
  if (showAngleSlider) {
    val angleSlider = new ScalaValueControl(0, 90, "property.ramp-angle".translate, "0.0".literal, "units.degrees".translate,
      () => model.surfaceSegments(1).getUnitVector.getAngle.toDegrees, value => model.setRampAngle(value.toRadians), model.surfaceSegments(1).addListener)
