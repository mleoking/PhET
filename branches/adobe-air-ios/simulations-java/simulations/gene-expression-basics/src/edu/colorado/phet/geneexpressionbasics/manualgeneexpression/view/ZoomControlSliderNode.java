 ...
 * controlling the zoom factor.  It looks like a vertically oriented slider
 * with some tick marks and some buttons for zooming in and out.
 *
 * @author John Blanco
 */
public class ZoomControlSliderNode extends PNode {
    ...
    private Property<Double> zoomFactor;
    ...
    public ZoomControlSliderNode( Dimension2D size, final Property<Double> zoomFactor, double minZoom, double maxZoom, int steps ) {

        // Parameter checking.
        assert zoomFactor.get() >= minZoom && zoomFactor.get() <= maxZoom;
        ...
        sliderTrack.addInputEventListener( new PBasicInputEventHandler() {
            public void mouseReleased( final PInputEvent event ) {
                zoomFactor.set( trackPosToZoomFactor( event.getPositionRelativeTo( sliderTrack ).getY() ) );
            }
        } );
        ...
        Shape sliderKnobShape = new RoundRectangle2D.Double( -size.getWidth() / 2, -sliderKnobHeight / 2,
                                                             size.getWidth(), sliderKnobHeight, 8, 8 );
        ...
        zoomInButton.addInputEventListener( new PBasicInputEventHandler() {
            ...
            public void mouseReleased( final PInputEvent event ) {
                ...
                if ( zoomFactor.get() < ZoomControlSliderNode.this.maxZoom ) {
                    if ( ZoomControlSliderNode.this.maxZoom - zoomFactor.get() > buttonZoomAmt ) {
                        zoomFactor.set( zoomFactor.get() + buttonZoomAmt );
                    }
                    else {
                        zoomFactor.set( ZoomControlSliderNode.this.maxZoom );
                    }
                }
            }
        } );
        ...
        zoomOutButton.addInputEventListener( new PBasicInputEventHandler() {
            ...
            public void mouseReleased( final PInputEvent event ) {
                ...
                if ( zoomFactor.get() > ZoomControlSliderNode.this.minZoom ) {
                    if ( zoomFactor.get() - ZoomControlSliderNode.this.minZoom > buttonZoomAmt ) {
                        zoomFactor.set( zoomFactor.get() - buttonZoomAmt );
                    }
                    else {
                        zoomFactor.set( ZoomControlSliderNode.this.minZoom );
                    }
                }
            }
        } );
        ...
        // Observe the zoom factor and change the knob position when changes
        // occur.
        zoomFactor.addObserver( new VoidFunction1<Double>() {
            public void apply( Double zoomFactor ) {
                updateSliderKnobPosition( zoomFactor );
            }
        } );

        // Initialize slider knob position.
        updateSliderKnobPosition( zoomFactor.get() );
    }
    ...
    private void updateSliderKnobPosition( double zoomFactor ) {
        sliderKnob.setOffset( sliderKnob.getOffset().getX(), zoomFactorToTrackPos( zoomFactor ) );
    }
    ...
    private void handleSliderKnobDragEvent( PInputEvent event ) {
        ...
        double newZoomValue = zoomFactor.get() + zoomDelta;

        newZoomValue = MathUtil.clamp( minZoom, newZoomValue, maxZoom );

        zoomFactor.set( newZoomValue );
    }
    ...
    private PNode createZoomButton( double size, char labelCharacter ) {
        Shape zoomButtonShape = new RoundRectangle2D.Double( 0, 0, size, size, 8, 8 );
