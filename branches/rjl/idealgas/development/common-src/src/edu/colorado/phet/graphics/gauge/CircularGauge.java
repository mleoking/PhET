...
public class CircularGauge extends AbstractGauge {
    ...
    public CircularGauge( int x, int y, float  min, float  max, float  amount, float  length ) {
        this( x, y, (int)length, (int)length, min, max, amount, length / 2, 5, 9 );
    }

    public CircularGauge( int x, int y, int width, int height,
                          float  min, float  max,
    ...
    public void drawButton( Graphics2D g ) {
        ...
        g.fillRect( (int)p.x - buttonWidth, (int)p.y - buttonWidth,
    ...
    public void drawNeedle( Graphics2D g ) {
        ...
        float  theta = amt / ( max - min ) * (float)Math.PI - (float)( Math.PI / 2 );
