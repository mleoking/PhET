<?php
	...
	if($_SERVER['QUERY_STRING']) {
		...
		display_desc("total number of messages for " . $simName);
		...
		display_desc("total number of messages for " . $simName . " (non-dev)");
		...
		display_desc("total number of sim runs for " . $simName . " (non-dev)");
		display_query("SELECT SQL_NO_CACHE SUM(session.sim_sessions_since) FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false)");
		
		display_desc("total number of runs for " . $simName . " since start of Jan 2009 (non-dev)");
		display_query("SELECT SQL_NO_CACHE SUM(session.sim_sessions_since) FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.timestamp >= '2009-01-01' AND session.sim_name = x.id AND session.sim_dev = false)");
		...
		display_desc("fixed sim runs by version and time-frame (non-dev)");
		display_query("SELECT SQL_NO_CACHE CONCAT(totals.major, '.', totals.minor) AS version, totals.val AS total, last_year.val AS last_year, last_month.val AS last_month, last_week.val AS last_week, last_day.val AS last_day FROM ( SELECT runs.sim_major_version AS major, runs.sim_minor_version AS minor, COUNT(runs.id) as val FROM ( SELECT versions.sim_major_version, versions.sim_minor_version, sim_sessions.id FROM (( SELECT DISTINCT session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false) ORDER BY sim_major_version, sim_minor_version ) AS versions) LEFT JOIN (( SELECT session.id AS id, session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false) ) AS sim_sessions) ON (sim_sessions.sim_major_version = versions.sim_major_version AND sim_sessions.sim_minor_version = versions.sim_minor_version) ) AS runs GROUP BY runs.sim_major_version, runs.sim_minor_version ORDER BY runs.sim_major_version, runs.sim_minor_version ) AS totals, ( SELECT runs.sim_major_version AS major, runs.sim_minor_version AS minor, COUNT(runs.id) as val FROM ( SELECT versions.sim_major_version, versions.sim_minor_version, sim_sessions.id FROM (( SELECT DISTINCT session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false) ORDER BY sim_major_version, sim_minor_version ) AS versions) LEFT JOIN (( SELECT session.id AS id, session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false AND session.timestamp >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 YEAR)) ) AS sim_sessions) ON (sim_sessions.sim_major_version = versions.sim_major_version AND sim_sessions.sim_minor_version = versions.sim_minor_version) ) AS runs GROUP BY runs.sim_major_version, runs.sim_minor_version ORDER BY runs.sim_major_version, runs.sim_minor_version ) AS last_year, ( SELECT runs.sim_major_version AS major, runs.sim_minor_version AS minor, COUNT(runs.id) as val FROM ( SELECT versions.sim_major_version, versions.sim_minor_version, sim_sessions.id FROM (( SELECT DISTINCT session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false) ORDER BY sim_major_version, sim_minor_version ) AS versions) LEFT JOIN (( SELECT session.id AS id, session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false AND session.timestamp >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 MONTH)) ) AS sim_sessions) ON (sim_sessions.sim_major_version = versions.sim_major_version AND sim_sessions.sim_minor_version = versions.sim_minor_version) ) AS runs GROUP BY runs.sim_major_version, runs.sim_minor_version ORDER BY runs.sim_major_version, runs.sim_minor_version ) AS last_month, ( SELECT runs.sim_major_version AS major, runs.sim_minor_version AS minor, COUNT(runs.id) as val FROM ( SELECT versions.sim_major_version, versions.sim_minor_version, sim_sessions.id FROM (( SELECT DISTINCT session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false) ORDER BY sim_major_version, sim_minor_version ) AS versions) LEFT JOIN (( SELECT session.id AS id, session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false AND session.timestamp >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 WEEK)) ) AS sim_sessions) ON (sim_sessions.sim_major_version = versions.sim_major_version AND sim_sessions.sim_minor_version = versions.sim_minor_version) ) AS runs GROUP BY runs.sim_major_version, runs.sim_minor_version ORDER BY runs.sim_major_version, runs.sim_minor_version ) AS last_week, ( SELECT runs.sim_major_version AS major, runs.sim_minor_version AS minor, COUNT(runs.id) as val FROM ( SELECT versions.sim_major_version, versions.sim_minor_version, sim_sessions.id FROM (( SELECT DISTINCT session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false) ORDER BY sim_major_version, sim_minor_version ) AS versions) LEFT JOIN (( SELECT session.id AS id, session.sim_major_version AS sim_major_version, session.sim_minor_version AS sim_minor_version FROM session, (SELECT id FROM sim_name WHERE name = '" . $simName . "') AS x WHERE (session.sim_name = x.id AND session.sim_dev = false AND session.timestamp >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 DAY)) ) AS sim_sessions) ON (sim_sessions.sim_major_version = versions.sim_major_version AND sim_sessions.sim_minor_version = versions.sim_minor_version) ) AS runs GROUP BY runs.sim_major_version, runs.sim_minor_version ORDER BY runs.sim_major_version, runs.sim_minor_version ) AS last_day WHERE ( totals.major = last_year.major AND totals.minor = last_year.minor AND totals.major = last_month.major AND totals.minor = last_month.minor AND totals.major = last_week.major AND totals.minor = last_week.minor AND totals.major = last_day.major AND totals.minor = last_week.minor ) ORDER BY totals.major, totals.minor;");
		*/
		...
		display_query("SELECT SQL_NO_CACHE DISTINCT sim_name.name AS sim_name, SUM(session.sim_sessions_since) AS sim_runs FROM session, sim_name WHERE (timestamp > '2007-01-01' AND timestamp < '2007-02-01' AND sim_name.id = session.sim_name AND session.sim_dev = false) GROUP BY sim_name ORDER BY sim_name;");
		
		display_desc("number of times each sim has been run (non-dev)");
		display_query("SELECT SQL_NO_CACHE DISTINCT sim_name.name AS sim_name, SUM(session.sim_sessions_since) AS sim_runs FROM session, sim_name WHERE (sim_name.id = session.sim_name AND session.sim_dev = false) GROUP BY sim_name.name ORDER BY sim_name.name;");
		
		display_desc("number of times sims have been run by week");
		display_query("SELECT SQL_NO_CACHE YEARWEEK(timestamp), SUM(session.sim_sessions_since) FROM session GROUP BY YEARWEEK(timestamp);");
		...
		display_query("SELECT SQL_NO_CACHE DISTINCT YEAR(timestamp) AS year, MONTH(timestamp) AS month, DAY(timestamp) AS day, SUM(session.sim_sessions_since) AS sim_runs FROM session WHERE (timestamp > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 YEAR) AND sim_dev = false) GROUP BY YEAR(timestamp), MONTH(timestamp), DAY(timestamp);");
		display_query("SELECT SQL_NO_CACHE DISTINCT YEAR(timestamp) AS year, MONTH(timestamp) AS month, DAY(timestamp) AS day, SUM(session.sim_sessions_since) AS sim_runs FROM session WHERE (DATEDIFF(CURRENT_DATE, timestamp) < 365 AND sim_dev = false) GROUP BY YEAR(timestamp), MONTH(timestamp), DAY(timestamp);");
		
		display_desc("number of times pendulum-lab has been run, by language, since the start of 2009");
		display_query("SELECT SQL_NO_CACHE session.sim_locale_language, SUM(session.sim_sessions_since) FROM session, sim_name WHERE (session.sim_name = sim_name.id AND sim_name.name = 'pendulum-lab' AND session.timestamp > '2009-01-01') GROUP BY session.sim_locale_language ORDER BY session.sim_locale_language;");
		
		display_desc("number of times each version of pendulum-lab was run within the last month");
		display_query("SELECT SQL_NO_CACHE session.sim_major_version, session.sim_minor_version, SUM(session.sim_sessions_since) FROM session, sim_name WHERE (session.sim_name = sim_name.id AND sim_name.name = 'pendulum-lab' AND session.sim_dev = 0 AND session.timestamp > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 31 DAY)) GROUP BY session.sim_major_version, session.sim_minor_version ORDER BY COUNT(session.id) DESC;");
		...
		display_desc("number of times each simplified OS has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE simplified_os.name, SUM(session.sim_sessions_since) FROM session, simplified_os WHERE (session.host_simplified_os = simplified_os.id AND session.sim_dev = false) GROUP BY simplified_os.name ORDER BY COUNT(session.id) DESC;");
		
		display_desc("number of times each deployment type has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE deployment.name, SUM(session.sim_sessions_since) FROM session, deployment WHERE (session.sim_deployment = deployment.id AND session.sim_dev = false) GROUP BY deployment.name ORDER BY COUNT(session.id) DESC;");
		
		display_desc("number of times each (non-null) distribution tag has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE distribution_tag.name, SUM(session.sim_sessions_since) FROM session, distribution_tag WHERE (distribution_tag.id = session.sim_distribution_tag AND distribution_tag.name IS NOT NULL AND session.sim_dev = false) GROUP BY distribution_tag.name ORDER BY COUNT(session.id) DESC;");
		...
		display_query("SELECT SQL_NO_CACHE YEAR(session.timestamp), MONTH(session.timestamp), SUM(session.sim_sessions_since) FROM session, (SELECT id FROM distribution_tag WHERE name = 'Wiley') AS x WHERE (session.sim_distribution_tag = x.id AND session.sim_dev = false) GROUP BY YEAR(session.timestamp), MONTH(session.timestamp);");
		
		display_desc("number of times each flash domain has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE flash_domain.name, SUM(session.sim_sessions_since) FROM session, session_flash_info, flash_domain WHERE (session.id = session_flash_info.session_id AND session_flash_info.host_flash_domain = flash_domain.id AND sim_dev = false) GROUP BY flash_domain.name ORDER BY COUNT(session.id) DESC;");
		
		display_desc("number of times each flash OS has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE flash_os.name, SUM(session.sim_sessions_since) FROM session, session_flash_info, flash_os WHERE (session.id = session_flash_info.session_id AND session_flash_info.host_flash_os = flash_os.id AND sim_dev = false) GROUP BY flash_os.name ORDER BY COUNT(session.id) DESC;");
		
		display_desc("number of times each java OS has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE java_os_name.name, java_os_version.name, java_os_arch.name, SUM(session.sim_sessions_since) FROM session, session_java_info, java_os_name, java_os_version, java_os_arch WHERE (session.id = session_java_info.session_id AND session_java_info.host_java_os_name = java_os_name.id AND session_java_info.host_java_os_version = java_os_version.id AND session_java_info.host_java_os_arch = java_os_arch.id AND session.sim_dev = false) GROUP BY java_os_name.name, java_os_version.name, java_os_arch.name ORDER BY COUNT(session.id) DESC;");
		
		display_desc("number of times each java timezone has been seen (non-dev)");
		display_query("SELECT SQL_NO_CACHE java_timezone.name, SUM(session.sim_sessions_since) from session, session_java_info, java_timezone WHERE (session.id = session_java_info.session_id AND session_java_info.host_java_timezone = java_timezone.id AND session.sim_dev = false) GROUP BY java_timezone.name ORDER BY COUNT(session.id) DESC;");
