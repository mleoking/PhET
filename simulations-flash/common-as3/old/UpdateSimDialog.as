...
class edu.colorado.phet.flashcommon.old.UpdateSimDialog extends edu.colorado.phet.flashcommon.old.CommonDialog {
	...
	public function UpdateSimDialog( versionMajor : Number, versionMinor : Number, versionDev : Number, versionRevision : Number, simAskLaterDays : Number ) {
		super( "updateSim", _level0.SimStrings.get( "NewVersionAvailable", "New Version Available" ) );
		...
		var str : String = "";
		str += SimStrings.get("CurrentVersionIs", "Your current version of {0} is {1}.", ["<b>" + common.getSimTitle() + "</b>", common.getShortVersionString()]) + "\n";
		str += SimStrings.get("NewerVersionIs", "A newer version {0} is available.", ["(" + common.zeroPadVersion(versionMajor, versionMinor) + ")"]) + "\n";
		
		var notUpdateStr = "<p align='center'><font color='#880000'>" + SimStrings.get("NotUpdateSim","This simulation cannot be updated automatically.") + "</font></p>";
		...
		if((common.fromFullInstallation() && common.updateHandler.simTimestamp + 7200 < common.updateHandler.installerTimestamp) || common.getSimName() == "flash-common-strings") {
			...
			str += SimStrings.get("PhETInstallation", defaultStr, ["asfunction:_level0.common.openExternalLink,http://" + FlashCommon.getMainServer() + "/get_phet/full_install.php"]);
		...
		str += "\n";
		str += SimStrings.get("UpdateOptionsAvailable", "Update options are available under <i>Preferences</i>.");
		...
		tryButton = new JButton(SimStrings.get("TryNow", "Try it now"));
		...
		askLaterButton = new JButton(SimStrings.get("AskLater", "Ask me later"));
		...
		skipButton = new JButton(SimStrings.get("PrivacySkip", "Skip this update"));
