...
class edu.colorado.phet.flashcommon.old.UpdateHandler {
	...
	public function showUpdateError() {
		if(!manual) { return; }
		var str : String = "";
		str += SimStrings.get("VersionError1", "An error was encountered while trying to obtain version information.") + "\n";
		str += SimStrings.get("VersionError2", "Please try again later, or visit <a href='{0}'>http://" + FlashCommon.getMainServer() + "</a>", ["asfunction:_level0.common.openExternalLink,http://" + FlashCommon.getMainServer() + ""]);
		CommonDialog.openMessageDialog( SimStrings.get("Error", "Error"), str, false );
	}
	
	public function showSimUpToDate() {
		if(!manual) { return; }
		var str : String = "";
		str += SimStrings.get("MostCurrentVersion", "You have the most current version ({0}) of {1}.", [common.getVersionString(), common.getSimTitle()]);
		CommonDialog.openMessageDialog( SimStrings.get("UpToDate", "Up To Date"), str, true );
	}
	
	public function showInstallationUpToDate() {
		if(!manual) { return; }
		var str : String = "";
		str += SimStrings.get("MostCurrentVersion", "You have the most current version ({0}) of {1}.", [FlashCommon.dateString(FlashCommon.dateOfSeconds(common.getInstallerCreationTimestamp())), "PhET Offline Website Installer"]);
		CommonDialog.openMessageDialog( SimStrings.get("UpToDate", "Up To Date"), str, true );
	}
