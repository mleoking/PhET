...
<?php if ( get_option( 'comment_registration' ) && !$user_ID ) : ?>
    <p><?php _e( 'You must be', 'yashfa' );?> <a
            href="<?php echo get_option( __( 'siteurl', 'yashfa' ) ); ?>/wp-login.php?redirect_to=<?php echo urlencode( get_permalink( ) ); ?>"><?php _e( 'logged in', 'yashfa' );?></a><?php _e( 'to post a comment.', 'yashfa' );?>
    </p>
<?php else : ?>
    <form action="<?php echo get_option( __( 'siteurl', 'yashfa' ) ); ?>/wp-comments-post.php" method="post"
          id="commentform">
        <div class="oneform">
        <?php if ( $user_ID ) : ?>
            <p><?php _e( 'Logged in as', 'yashfa' );?> <a
                    href="<?php echo get_option( __( 'siteurl', 'yashfa' ) ); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>.
                <a href="<?php echo wp_logout_url( get_permalink( ) ); ?>"
                   title="<?php _e( 'Log out of this account', 'yashfa' );?>"><?php _e( 'Log out &raquo;', 'yashfa' );?></a>
            </p>
        <?php else : ?>
            <p>
                <label for="author">
                    <small><?php _e( 'Name', 'yashfa' );?></label>
            ...
            <p>
                <label for="email">
                    <small><?php _e( 'Mail', 'yashfa' )?></small>
                </label>
            ...
            <p>
                <label for="url">
                    <small><?php _e( 'Website', 'yashfa' )?></small>
                </label>
        ...
        <p>
            <input name="submit" type="submit" id="submit" tabindex="5"
                   value="<?php _e( 'Submit Comment', 'yashfa' )?>"/>
