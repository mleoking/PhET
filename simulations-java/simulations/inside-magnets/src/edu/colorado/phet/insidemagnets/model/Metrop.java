...
//     * ****************************************************************
//     */
//    int lookvort( int N, int i1, int[][] nbrs, double[] phi )
///*
...
//     * ****************************************************************
//     */
//    int Metrop( int N, float[] x, float[] y, int[][] nbrs, int[] phase, double[] sx, double[] sy, double[] sz, double[] phi, int[] vort,
//                double[] J, double Bx, double By, double[] bx, double[] by, double Ka, int bc, double rt, double sinc, double[] e, double[] mx, double[] my, double[] mz, int[] nv ) {
///*
...
//        extern
//        double ONEPI, TWOPI;
//        int kyes, i, isite, j, jp, nbr1, nbr2, nbr3, nbr4, np[
//        4];
...
//
//            nbr1 = nbrs[isite][0];
//            np[0] = isite;  /* neighboring plaquettes. */
//            nbr2 = nbrs[isite][1];
//            np[1] = nbr2;
//            nbr3 = nbrs[isite][2];
//            np[2] = nbrs[nbr2][3];
...
//                        if ( dv >= 2 ) { draw_plus( win, gcbrt, X1, Y1, size * XScale, size * YScale ); }
//                        else if ( dv == 1 ) { draw_minus( win, gcbrt, X1, Y1, size * XScale ); }
//                        else if ( dv == -1 ) { draw_minus( win, gcclear, X1, Y1, size * XScale ); }
//                        else if ( dv <= -2 ) {
...
//     * *****************************************
//     */
//    void averages( int nsamp, int Nsites, double beta, double e, double M, double[] C, double[] X ) {
//        double esum, e2sum, Msum, M2sum;
...
//
//        r = 1.0 / (double) nsamp;
//        rN = 1.0 / (double) Nsites;
//        eave = r * esum;
//        e2ave = r * e2sum;
//        Mave = r * Msum;
//        M2ave = r * M2sum;
//        C[0] = beta * ( beta * rN ) * ( e2ave - eave * eave );
//        X[0] = ( beta * rN ) * ( M2ave - Mave * Mave );
//
//        return;
//    }
//
