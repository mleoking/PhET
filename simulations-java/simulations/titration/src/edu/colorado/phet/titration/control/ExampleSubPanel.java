...
package edu.colorado.phet.titration.control;
...
import edu.colorado.phet.titration.model.ExampleModelElement.ExampleModelElementListener;
...
public class ExampleSubPanel extends JPanel {
    ...
    public ExampleSubPanel( final ExampleModelElement modelElement ) {
        ...
        // Title
        JLabel titleLabel = new JLabel( TitrationStrings.TITLE_EXAMPLE_CONTROL_PANEL );
        ...
        double min = 0;
        double max = 360;
        String label = TitrationStrings.LABEL_ORIENTATION;
        String valuePattern = "##0";
        String units = TitrationStrings.UNITS_ORIENTATION;
    ...
    private void setPosition( Point2D p ) {
        String s = TitrationStrings.LABEL_POSITION + " (" + (int) p.getX() + "," + (int) p.getY() + ")";
