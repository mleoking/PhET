...
public class TutorialCanvas extends PSwingCanvas {
    ...
    static {
        try {
            backgroundImage = ImageLoader.loadBufferedImage( "images/background.jpg" );
        }
        catch( IOException e ) {
            e.printStackTrace();
        }
...
//        System.out.println( "Page.addchild, layer=" + getLayer().getChildrenReference().size() + ", " + getLayer().getChildrenReference() );
    ...
    public void playHarp() {
        JSAudioPlayer.playNoBlock( BasicTutorialCanvas.class.getClassLoader().getResource( "audio/upstroke.wav" ) );
    }
