...
class CoordinateFrameNode(val model: RampModel, coordinateSystemModel: CoordinateSystemModel, val transform: ModelViewTransform2D) extends PNode {
  ...
  val yAxis = new AxisNodeWithModel(transform, "y", yAxisModel, coordinateSystemModel.adjustable, PI / 2, PI)
  addChild(yAxis)

  val xAxisModel = new SynchronizedAxisModel(0, 7, false, model.coordinateFrameModel)
  val xAxis = new AxisNodeWithModel(transform, "x", xAxisModel, coordinateSystemModel.adjustable, 0, PI / 2)
  addChild(xAxis)
