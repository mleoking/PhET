...
public class DropperNode extends PhetPNode {
    ...
    private static final double LABEL_Y_OFFSET = 40 * IMAGE_SCALE; // y offset of the label's center from the image's center

    public DropperNode( final Property<Solute> solute, final Property<SoluteForm> soluteFormProperty ) {

        // Image file is assumed to be oriented with dropper hole pointing down.
        ...
        solute.addObserver( new SimpleObserver() {
            public void update() {
                ...
                labelNode.setOffset( -( labelNode.getFullBoundsReference().getWidth() / 2 ),
                                     -( labelNode.getFullBoundsReference().getHeight() / 2 ) + LABEL_Y_OFFSET );
            }
        } );
