...
public class LayerNode extends PNode {
    public LayerNode( GeneralPath topLine, GeneralPath bottomLine, Color color, double depth ) {
//        Rectangle2D.Double aDouble = new Rectangle2D.Double( 0, 0, 400, 100 );
        DoubleGeneralPath faceShape = new DoubleGeneralPath( 0, 0 );
        faceShape.append( topLine, true );
        faceShape.lineToRelative( 0, depth );

        Shape txBottom = AffineTransform.getTranslateInstance( 0, depth ).createTransformedShape( bottomLine );

        faceShape.append( new ReversePathIterator( txBottom.getPathIterator( new AffineTransform() ) ), true );
