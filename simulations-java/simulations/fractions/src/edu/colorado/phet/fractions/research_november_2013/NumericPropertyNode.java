...
public class NumericPropertyNode<T extends Number> extends PNode {
    ...
    public NumericPropertyNode( ObservableProperty<T> property, final Function.LinearFunction valueToY, final Property<Function.LinearFunction> timeToX, VoidFunction1<VoidFunction0> addTickListener ) {
        final VoidFunction0 update = new VoidFunction0() {
            public void apply() {
                removeAllChildren();
                for ( int i = 0; i < timestamps.size(); i++ ) {
                    Pair<T, Long> timestamp = timestamps.get( i );
                    long nextTime = i == timestamps.size() - 1 ? System.currentTimeMillis() : timestamps.get( i + 1 )._2;
