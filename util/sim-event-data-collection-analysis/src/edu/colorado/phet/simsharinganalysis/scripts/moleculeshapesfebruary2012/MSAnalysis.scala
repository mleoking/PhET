...
object MSAnalysis extends StateMachine[SimState] {
  ...
  class Report(val log: Log) {
    ...
    val numMoleculeTransitions = statePairs.filter(pair => pair._1.tab1.molecule != pair._2.tab1.molecule).length
    val numViewTransitions = statePairs.filter(pair => pair._1.tab1.view != pair._2.tab1.view).length
