...
class BAMStateMachine extends StateMachine[SimState] {
  ...
  def init(time: Long) = SimState(time, 0, TabState(Nil), TabState(Nil), TabState(Nil))
  ...
  def nextState(state: SimState, e: Entry) = {
    ...
    else if ( e.messageType == "model" && e.component == "collectionBox" && state.tab == 0 ) state.copy(time = e.time, tab0 = state.tab0.collect(e("collectionBoxCommonName")))
    else if ( e.messageType == "model" && e.component == "collectionBox" && state.tab == 1 ) state.copy(time = e.time, tab1 = state.tab1.collect(e("collectionBoxCommonName")))
    else if ( e.messageType == "model" && e.component == "collectionBox" && state.tab == 2 ) state.copy(time = e.time, tab2 = state.tab2.collect(e("collectionBoxCommonName")))
