...
public class DensitySameDensityMode extends Mode {
    ...
    override public function init(): void {
        ...
        var block2: Block = Block.newBlockDensityMass( density, 3, 0, 0, DensityConstants.BLUE, model, Material.CUSTOM );
        block2.setPosition( -DensityConstants.POOL_WIDTH_X / 2 - block1.getWidth(), block2.getHeight() / 2 );
        model.addDensityObject( block2 );

        var block3: Block = Block.newBlockDensityMass( density, 2, 0, 0, DensityConstants.GREEN, model, Material.CUSTOM );
        block3.setPosition( DensityConstants.POOL_WIDTH_X / 2, block3.getHeight() / 2 );
        model.addDensityObject( block3 );

        var block4: Block = Block.newBlockDensityMass( density, 1, 0, 0, DensityConstants.RED, model, Material.CUSTOM );
