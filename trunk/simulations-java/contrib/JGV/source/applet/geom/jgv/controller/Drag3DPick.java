...
public class Drag3DPick extends Drag3DPickable {

  CameraCanvas cv;

  public Drag3DPick(Camera3D cam, HMatrix3D toAdjust, CameraCanvas toRepaint) {
    super(cam, toAdjust, toRepaint);
    cv = toRepaint;
  }

  public Drag3DPick(Camera3D cam, CameraCanvas toRepaint) {
  ...
  public boolean mouseDrag(MouseEvent ev, int x, int y) {
    ...
    if (cv.tree != null) {
      ...
      cv.tree.render.picked = picker.getPickSet();
    }
    toRepaint.repaint();
    return true;
  }
  ...
  public boolean mouseMoved(MouseEvent ev, int x, int y) {
    ...
    if (cv.tree != null) {
      ...
      } else if (f.parent.getAction() == null) {
        cv.tree.render.picked = null;
      } else {
