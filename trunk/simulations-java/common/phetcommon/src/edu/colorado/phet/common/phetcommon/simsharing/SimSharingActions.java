// Copyright 2002-2011, University of Colorado
package edu.colorado.phet.common.phetcommon.simsharing;

/**
 * Common actions that are reported via sim sharing.
 *
 * @author Chris Malley (cmalley@pixelzoom.com)
 */
public class SimSharingActions {

    public static final String ACTIVATED = "activated";
    public static final String EXITED = "exited";
    public static final String DEACTIVATED = "deactivated";
    public static final String DEICONIFIED = "deiconified";
    public static final String ICONIFIED = "iconified";
    public static final String PRESSED = "pressed";
    public static final String RESIZED = "resized";
    public static final String SELECTED = "selected";//TODO: replace usages of this with PRESSED for JMenu and JMenuItem?
    public static final String TOGGLED = "toggled";
    public static final String WINDOW_SYSTEM_CLOSE_BUTTON_PRESSED = "closeButtonPressed"; //TODO change this literal to "windowSystemCloseButtonPressed"?
}
