...
public class AlphaFixer2 extends LookupTable {
    ...
    public int[] lookupPixel( int[] src, int[] dst ) {
        //edu.colorado.phet.util.Debug.traceln("Looking up source="+toString(src));
        //edu.colorado.phet.util.Debug.traceln("Match="+toString(match));
        if( isWhite( src ) ) {
            //edu.colorado.phet.util.Debug.traceln("Returning alpha.");
            for( int i = 0; i < dst.length; i++ ) {
                dst[i] = ALPHA[i];
            }
            return dst;
        }
        //edu.colorado.phet.util.Debug.traceln("Returning source.");
        return src;
    }
