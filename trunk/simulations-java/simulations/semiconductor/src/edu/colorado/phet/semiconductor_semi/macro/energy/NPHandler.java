...
import edu.colorado.phet.semiconductor_semi.macro.energy.bands.*;
...
import edu.colorado.phet.semiconductor_semi.macro.energy.statemodels.TypeCriteria;
import edu.colorado.phet.semiconductor_semi.macro.energy.transitions.ExitRight;
...
public class NPHandler implements ModelElement, ModelCriteria {
    ...
    public NPHandler( EnergySection energySection ) {
        ...
        EnergyCell cell = energySection.bandSetAt( 1 ).bandAt( 1 ).energyLevelAt( DopantType.P.getNumFilledLevels() ).cellAt( 1 );
        ...
        EnergyCell topN = energySection.bandSetAt( 1 ).bandAt( 2 ).energyLevelAt( DopantType.N.getNumFilledLevels() ).cellAt( 0 );
...
//        System.out.println( "mov = " + mov );
    ...
    private ModelElement getNPForward() {
        ...
        ExciteForConduction exciteP = pnForward.exciteP( 1, 1 );
...
class NPForwardClear extends DefaultStateDiagram {
    private ModelElement npForward;
    private int topLevel = DopantType.P.getNumFilledLevels();

    public NPForwardClear( EnergySection energySection, ModelElement npForward ) {
        super( energySection );
