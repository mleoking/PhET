    ...
    class FullCircuit extends MNA.AbstractCircuit {
        ...
        CompanionCircuit getCompanionModel(double dt) {
            ...
            for (HasCompanionModel c : sourceElements) {
                ...
                for (MNA.Battery battery : cm.batteries) b.add(battery);
                for (MNA.Resistor resistor : cm.resistors) r.add(resistor);
                for (MNA.CurrentSource currentSource : cm.currentSources) cs.add(currentSource);
            }
