...
public class Crystal extends Body implements Collidable {
    ...
    public Object clone() {
        ...
        for ( int i = 0; i < ions.size(); i++ ) {
            ...
            Ion newIon = ionFactory.create( ion.getClass(),
                                            ...
                                            new MutableVector2D( ion.getVelocity() ),
                                            new MutableVector2D( ion.getAcceleration() ) );
            if ( this.getSeed() == ion ) {
                newSeed = newIon;
            }
            newIons.add( newIon );
    ...
    public void releaseIon( double dt ) {
        ...
        MutableVector2D v = determineReleaseVelocity( ionToRelease );
     ...
     */
    private MutableVector2D determineReleaseVelocity( Ion ionToRelease ) {
        MutableVector2D releaseVelocity = null;
        ...
        if ( openSites.size() > 1 ) {
            for ( int i = 0; i < openSites.size() - 1; i++ ) {
                ...
                MutableVector2D v1 = new MutableVector2D( p1.getX() - ionToRelease.getPosition().getX(),
                ...
                for ( int j = i + 1; j < openSites.size(); j++ ) {
                    ...
                    MutableVector2D v2 = new MutableVector2D( p2.getX() - ionToRelease.getPosition().getX(),
        ...
        MutableVector2D v = new MutableVector2D( point2D.getX() - ionToRelease.getPosition().getX(),
