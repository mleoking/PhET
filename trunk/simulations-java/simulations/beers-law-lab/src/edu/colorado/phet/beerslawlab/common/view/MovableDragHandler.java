...
public class MovableDragHandler extends SimSharingDragHandler {
    ...
    private static Vector2D constrainToBounds( double x, double y, Rectangle2D bounds ) {
        Vector2D vConstrained;
        if ( bounds != null && !bounds.contains( x, y ) ) {
            ...
            vConstrained = new Vector2D( xConstrained, yConstrained );
        }
        else {
            vConstrained = new Vector2D( x, y );
        }
        return vConstrained;
    }
}
