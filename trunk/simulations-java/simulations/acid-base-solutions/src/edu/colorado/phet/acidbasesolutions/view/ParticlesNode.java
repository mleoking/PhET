...
public class ParticlesNode extends PComposite {
    ...
    private static final Color DEFAULT_H3O_COLOR = ABSConstants.H3O_PARTICLES_COLOR;
    private static final Color DEFAULT_OH_COLOR = ABSConstants.OH_PARTICLES_COLOR;
    ...
    public ParticlesNode( Solution solution, PBounds containerBounds ) {
        super();
        setPickable( false );
        ...
        if ( AcidBaseSolutionsApplication.getInstance().isDeveloperControlsEnabled() ) {
    ...
    private int getNumberOfH3OParticles( double pH ) {
        return (int) ( _solution.getConcentrationH3O() * ( _numberOfParticlesAtPH7 / 2 ) / 1E-7 );
    }
    
    private int getNumberOfOHParticles( double pH ) {
        return (int) ( _solution.getConcentrationOH() * ( _numberOfParticlesAtPH7 / 2 ) / 1E-7 );
    }
