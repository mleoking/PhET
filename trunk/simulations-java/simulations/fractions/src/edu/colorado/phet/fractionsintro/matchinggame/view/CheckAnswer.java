...
public @Data class CheckAnswer extends F<MatchingGameState, MatchingGameState> {
    @Override public MatchingGameState f( final MatchingGameState state ) {
        return state.getLeftScaleValue() == state.getRightScaleValue() ? state.withChecks( state.info.checks + 1 ).withMode( Mode.USER_CHECKED_CORRECT_ANSWER ) :
               state.withChecks( state.info.checks + 1 ).withMode( Mode.SHOWING_WHY_ANSWER_WRONG );
    }
}