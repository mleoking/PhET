 ...
 * position the slope manipulator on the graph. The intercept is fixed, but plotted.
 *
 * @author Chris Malley (cmalley@pixelzoom.com)
 */
public class GraphSlopeNode extends PhetPNode {

    //TODO Most of this class and inner class was copied from GraphSlopeInterceptNode, should be able to combine.
    public GraphSlopeNode( final LineGameModel model, final GameAudioPlayer audioPlayer, PDimension challengeSize ) {

        // Since y-intercept is fixed, make sure that the guess has the same y-intercept as the answer
        assert ( model.challenge.get().answer.x1 == model.challenge.get().guess.get().x1 &&
                 model.challenge.get().answer.y1 == model.challenge.get().guess.get().y1 );
