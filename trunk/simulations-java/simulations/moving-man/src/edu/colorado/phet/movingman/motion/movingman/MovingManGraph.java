...
public class MovingManGraph extends MotionControlGraph {
    ...
    public MovingManGraph( PhetPCanvas pSwingCanvas, final ControlGraphSeries series, String label, String title, double min, double max,
                           boolean editable, final TimeSeriesModel timeSeriesModel,
                           final UpdateStrategy updateStrategy, double maxDomainValue, final UpdateableObject iPositionDriven ) {
        this( pSwingCanvas, series, label, title, min, max, editable, timeSeriesModel, updateStrategy, maxDomainValue, iPositionDriven, 5 );
    }
...
//                series.getTemporalVariable().removeAll( indices );
                    ...
                    for ( int i : indices ) {
                        series.getTemporalVariable().setTimeData( i, series.getTemporalVariable().getData( index ).getTime(), plotPT.getY() );
                    }
...
//        TimeData[] timeData = series.getTemporalVariable().getRecentSeries( 5 );
//        double sum =0;
//        int count=0;
