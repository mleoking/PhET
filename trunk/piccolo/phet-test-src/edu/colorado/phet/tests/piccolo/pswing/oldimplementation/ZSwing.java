 ...
 * Swing Components to a Jazz PNode.
 * <P>
 * Example: adding a swing JButton to a PNode:
 * <pre>
 ...
 * @author Lance E. Good
 */
public class ZSwing extends PNode implements Serializable, PropertyChangeListener {
     ...
     */
    public ZSwing( JComponent component ) {

        this.component = component;
        component.putClientProperty( VISUAL_COMPONENT_KEY, this );
        init( component );
        getSwingWrapper().add( component );
        component.revalidate();

//        reshape();
        reshape();
        addInputEventListener( new ZSwingEventHandler( this ) );

        PhetJComponentRepaintManager.MANAGER.put( this );
    ...
    public static class SwingWrapper extends JPanel {
        public SwingWrapper() {
    ...
    private static final SwingWrapper SWING_WRAPPER = new SwingWrapper();

    public static SwingWrapper getSwingWrapper() {
        return SWING_WRAPPER;
    }
     ...
     */
    protected void paint( PPaintContext renderContext ) {
...
//            PCanvas canvas = (PCanvas)renderContext.getDrawingSurface().getComponent();
            getSwingWrapper().add( component );
            component.revalidate();
        }
...
//        PCanvas.BasicRepaintManager manager = (PCanvas.ZBasicRepaintManager)RepaintManager.currentManager( component );
...
//            }
//            else {
//                parentsRef[i].repaint( (PBounds)repaintBounds.clone() );
//            }
//        }
//    }
    ...
    public static class PhetJComponentRepaintManager extends RepaintManager {
        ...
        public static PhetJComponentRepaintManager MANAGER = new PhetJComponentRepaintManager();

        public synchronized void addDirtyRegion( JComponent c, int x, int y, int w, int h ) {
            super.addDirtyRegion( c, x, y, w, h );
            if( table.containsKey( c ) ) {
                ZSwing phetJComponent = (ZSwing)table.get( c );
//                if( c.getComponentCount() == 0 && new Exception().getStackTrace().length < 75 ) {
//                if( c.getComponentCount() == 0 && new Exception().getStackTrace().length < 75 ) {
                if( true && new Exception().getStackTrace().length < 75 ) {
                    if( !dirty.contains( c ) ) {
        ...
        public void put( ZSwing phetJComponent ) {
        ...
        public void updateGraphics() {
            for( int i = 0; i < dirty.size(); i++ ) {//todo coalesce the paint messages.  Can some code from RepaintManager be reused?
                ZSwing swing = (ZSwing)dirty.get( i );
                swing.repaint();
            }
            dirty.clear();
        }
