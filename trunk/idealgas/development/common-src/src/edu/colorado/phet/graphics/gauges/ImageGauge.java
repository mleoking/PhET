...
public class ImageGauge extends IGauge {
    float  min;
    float  max;
    float  amount;
    float  length;
    ...
    public void paint( Graphics2D g ) {
        ...
        float  theta = (float)( amount / ( max - min ) * Math.PI );
        //util.Debug.traceln("theta="+theta);
        float  xD = (float)( length * Math.sin( theta ));
        float  yD = (float)( length * Math.cos( theta ));
