...
public class WiggleMe extends CompositePhetGraphic {
    ...
    private double frequency = 4.5;//in Hertz.
    private double amplitude = 15;//In pixels.
    ...
    public WiggleMe( Component component, BaseModel clock, String text, PhetGraphic phetGraphic ) {
        this( component, clock, text, new PhetGraphicTarget( phetGraphic ) );
    }

    public WiggleMe( final Component component, BaseModel clock, String text, Target t ) {
        this( component, clock, text, t, new Font( "Lucida Sans", Font.BOLD, 14 ), 1, 1 );
    }

    public WiggleMe( final Component component, BaseModel clock, String text, Target t, Font font, int dx, int dy ) {
        super( component );
        this.target = t;
        this.clock = clock;
...
//        clock.addClockTickListener( tickListener );
        Arrow arrow = new Arrow( new Point2D.Double( 50, 0 ), new Point2D.Double( 0, 0 ), 20, 20, 10 );
...
//        phetShapeGraphic.setLocation( 0, textGraphic.getHeight() + 5 );
    ...
    public void setVisible( boolean visible ) {
        ...
        if( visible ) {
            if( !clock.containsModelElement( tickListener ) ) {
                clock.addModelElement( tickListener );
            }
        }
        else {
            while( clock.containsModelElement( tickListener ) ) {
                clock.removeModelElement( tickListener );
            }
        }
    }
...
//        System.out.println( "tick" );
    ...
    public static class PhetGraphicTarget implements Target {
        ...
        public Point getLocation() {
            ...
            return new Point( targetLoc.x + target.getBounds().width, targetLoc.y );
//            return targetLoc;
        }
